...
Route::patch('/forum/threads/{thread}', [\App\Http\Controllers\ForumController::class, 'update'])->name('forum.thread.update')->middleware('auth');
...
Route::patch('/forum/replies/{reply}', [\App\Http\Controllers\ForumController::class, 'updateReply'])->name('forum.reply.update')->middleware('auth');

// Follow toggle
Route::post('/users/{user:username}/follow', function (\App\Models\User $user) {
    $me = auth()->user();
    if ($me->id === $user->id) {
    ...
    return response()->json([
        'following' => $following,
        'count'     => $user->fresh()->followers()->count(),
    ]);
})->name('users.follow')->middleware('auth');

// Block toggle
Route::post('/users/{user:username}/block', function (\App\Models\User $user) {
    $me = auth()->user();
    if ($me->id === $user->id) {
        return response()->json(['error' => 'Action impossible.'], 422);
    }
    $existing = \App\Models\Block::where('blocker_id', $me->id)->where('blocked_id', $user->id)->first();
    if ($existing) {
        $existing->delete();
        $blocked = false;
    } else {
        \App\Models\Block::create(['blocker_id' => $me->id, 'blocked_id' => $user->id]);
        $me->following()->detach($user->id);
...
// Feed (fil d'actualité — authentifié)
...
Route::prefix('admin')->name('admin.')->middleware(['auth', 'admin'])->group(function () {
    ...
    // One-shot : publier les articles de Babstaroi
    Route::get('/seed-babstaroi', function () {
        (new \Database\Seeders\BabstaroiSeeder())->run();
        return response('<pre style="background:#111;color:#8f8;padding:20px">✅ Articles de Babstaroi publiés. <a href="/blog" style="color:#C8522A">→ Voir le blog</a></pre>');
    })->name('seed-babstaroi');
});
