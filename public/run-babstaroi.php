<?php
// Script one-shot — supprimer après usage
...
$user = User::where('username', 'babstaroi')->first();
if (!$user) { die('❌ Utilisateur babstaroi introuvable.'); }

$exists = Post::where('user_id', $user->id)->where('title', 'like', "%Claude, Gemini%")->exists();
if ($exists) { die('⚠️ Articles déjà créés.'); }
...
echo '<pre style="background:#111;color:#8f8;padding:20px;font-family:monospace">✅ 2 articles publiés sous Babstaroi. <a href="/blog" style="color:#C8522A">→ Voir le blog</a></pre>';
