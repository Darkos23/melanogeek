...
@section('title', 'Modifier le projet — Portfolio')
...
<style>
    ...
    .pf-input:focus, .pf-textarea:focus { border-color:var(--terra); }
    ...
    .pf-cat-option:checked + .pf-cat-label { background:var(--terra-soft);border-color:var(--terra);color:var(--terra); }
    ...
    .pf-cover-preview { width:100%;height:100%;object-fit:cover;position:absolute;inset:0; }

    .pf-tags-hint { font-size:.72rem;color:var(--text-faint);margin-top:5px; }
    .pf-submit-row { display:flex;gap:12px;margin-top:32px; }
    .pf-btn-submit { flex:1;padding:14px 24px;border-radius:100px;background:var(--terra);color:white;border:none;font-family:var(--font-head);font-size:.95rem;font-weight:700;cursor:pointer;transition:background .2s; }
    .pf-btn-submit:hover { background:var(--accent); }
    .pf-btn-cancel { padding:14px 24px;border-radius:100px;background:var(--bg-card2);color:var(--text-muted);border:1px solid var(--border);text-decoration:none;font-size:.9rem;font-weight:600;font-family:var(--font-body);display:inline-flex;align-items:center;transition:all .2s; }
...
<div class="pf-form-page">
    ...
    <div class="pf-heading">Modifier le projet</div>
    <div class="pf-sub">Mets à jour les informations de ce projet dans ton portfolio.</div>
    ...
    <form method="POST" action="{{ route('portfolio.update', $item) }}" enctype="multipart/form-data">
        ...
        <div class="pf-card">
            ...
            <div class="pf-field">
                <label class="pf-label" for="title">Titre du projet</label>
                <input class="pf-input" id="title" name="title" type="text"
                    value="{{ old('title', $item->title) }}" required maxlength="120">
            </div>
            ...
            <div class="pf-field">
                <label class="pf-label" for="tags">Tags <span>(séparés par des virgules)</span></label>
                <input class="pf-input" id="tags" name="tags" type="text"
                    value="{{ old('tags', $item->tags ? implode(', ', $item->tags) : '') }}"
                    placeholder="branding, logo, afrique">
            ...
            <div class="pf-cat-grid">
                ...
                <div>
                    <input class="pf-cat-option" type="radio" name="category" id="cat_{{ $key }}"
                        value="{{ $key }}" {{ old('category', $item->category) === $key ? 'checked' : '' }}>
        ...
        <div class="pf-card">
            ...
            <div class="pf-cover-zone" onclick="document.getElementById('cover_image').click()">
                <input type="file" id="cover_image" name="cover_image" accept="image/*"
                    onchange="previewCover(this)" style="display:none;">
                <img class="pf-cover-preview" id="coverPreview"
                    src="{{ $item->cover_url ?? '' }}"
                    alt=""
                    style="{{ $item->cover_url ? '' : 'display:none;' }}">
                @if(!$item->cover_url)
                <div id="coverPlaceholder" style="display:flex;flex-direction:column;align-items:center;gap:10px;">
                    <div style="font-size:2rem;">🖼️</div>
                    <div style="font-size:.82rem;color:var(--text-muted);text-align:center;">
                        <strong style="color:var(--terra);">Choisir une nouvelle image</strong><br>
                        JPG, PNG — max 4 Mo
                    </div>
                </div>
                @else
                <div id="coverPlaceholder" style="display:none;"></div>
                @endif
            </div>
            @if($item->cover_url)
            <div style="margin-top:10px;font-size:.78rem;color:var(--text-muted);">
                ↑ Image actuelle. Clique pour en choisir une autre.
            </div>
            @endif
        </div>
        ...
        <div class="pf-submit-row">
            ...
            <button type="submit" class="pf-btn-submit">💾 Enregistrer les modifications</button>
        </div>
