...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,200px),1fr));gap:16px;margin-bottom:28px;">
    ...
    <div style="background:var(--bg-card);border:1px solid rgba(212,168,67,.25);border-radius:18px;padding:22px 24px;position:relative;overflow:hidden;">
        ...
        <div style="display:flex;align-items:flex-start;justify-content:space-between;position:relative;">
            <div>
                ...
                <div style="font-size:.75rem;color:var(--text-muted);margin-top:6px;">candidature{{ $pending->total() > 1 ? 's' : '' }} à traiter</div>
            </div>
    ...
    <div style="background:var(--bg-card);border:1px solid rgba(45,90,61,.3);border-radius:18px;padding:22px 24px;position:relative;overflow:hidden;transition:border-color .2s;{{ $currentStatus === 'approved' ? 'border-color:rgba(125,223,154,.4);' : '' }}" onmouseover="this.style.borderColor='rgba(125,223,154,.35)'" onmouseout="this.style.borderColor='{{ $currentStatus === 'approved' ? 'rgba(125,223,154,.4)' : 'rgba(45,90,61,.3)' }}'">
        ...
        <div style="display:flex;align-items:flex-start;justify-content:space-between;position:relative;">
            <div>
                <div style="font-size:.72rem;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:var(--text-muted);margin-bottom:10px;">Approuvées</div>
                <div style="font-family:var(--font-head);font-size:2.4rem;font-weight:800;color:#7DDF9A;line-height:1;">{{ $approved }}</div>
                <div style="font-size:.75rem;color:var(--text-muted);margin-top:6px;">créateur{{ $approved > 1 ? 's' : '' }} actif{{ $approved > 1 ? 's' : '' }}</div>
            </div>
            <div style="width:44px;height:44px;background:rgba(45,90,61,.2);border:1px solid rgba(125,223,154,.2);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1.3rem;flex-shrink:0;">✅</div>
        </div>
    ...
    <div style="background:var(--bg-card);border:1px solid rgba(224,85,85,.2);border-radius:18px;padding:22px 24px;position:relative;overflow:hidden;transition:border-color .2s;{{ $currentStatus === 'rejected' ? 'border-color:rgba(224,85,85,.4);' : '' }}" onmouseover="this.style.borderColor='rgba(224,85,85,.35)'" onmouseout="this.style.borderColor='{{ $currentStatus === 'rejected' ? 'rgba(224,85,85,.4)' : 'rgba(224,85,85,.2)' }}'">
        ...
        <div style="display:flex;align-items:flex-start;justify-content:space-between;position:relative;">
            <div>
                <div style="font-size:.72rem;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:var(--text-muted);margin-bottom:10px;">Refusées</div>
                <div style="font-family:var(--font-head);font-size:2.4rem;font-weight:800;color:#E05555;line-height:1;">{{ $rejected }}</div>
                <div style="font-size:.75rem;color:var(--text-muted);margin-top:6px;">candidature{{ $rejected > 1 ? 's' : '' }} refusée{{ $rejected > 1 ? 's' : '' }}</div>
            </div>
            <div style="width:44px;height:44px;background:rgba(224,85,85,.1);border:1px solid rgba(224,85,85,.2);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1.3rem;flex-shrink:0;">❌</div>
        </div>
...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:12px;">
    <div>
        <div style="font-family:var(--font-head);font-size:1.1rem;font-weight:800;color:var(--text);">
            ...
            @elseif($currentStatus === 'approved') ✅ Candidatures approuvées
            @else ❌ Candidatures refusées
            @endif
        </div>
        <div style="font-size:.78rem;color:var(--text-muted);margin-top:3px;">
            {{ $pending->total() }} résultat{{ $pending->total() > 1 ? 's' : '' }}
        </div>
    </div>
    <div style="display:flex;gap:6px;background:var(--bg);border:1px solid var(--border);border-radius:12px;padding:4px;">
        <a href="{{ request()->fullUrlWithQuery(['status' => 'pending']) }}"
           style="padding:6px 14px;border-radius:8px;font-family:var(--font-head);font-size:.78rem;font-weight:700;text-decoration:none;transition:all .2s;
                  {{ $currentStatus === 'pending' ? 'background:var(--bg-card);color:var(--gold);box-shadow:0 1px 4px rgba(0,0,0,.2);' : 'color:var(--text-muted);' }}">
            ⏳ En attente
        </a>
        <a href="{{ request()->fullUrlWithQuery(['status' => 'approved']) }}"
           style="padding:6px 14px;border-radius:8px;font-family:var(--font-head);font-size:.78rem;font-weight:700;text-decoration:none;transition:all .2s;
                  {{ $currentStatus === 'approved' ? 'background:var(--bg-card);color:#7DDF9A;box-shadow:0 1px 4px rgba(0,0,0,.2);' : 'color:var(--text-muted);' }}">
            ✅ Approuvées
        </a>
        <a href="{{ request()->fullUrlWithQuery(['status' => 'rejected']) }}"
           style="padding:6px 14px;border-radius:8px;font-family:var(--font-head);font-size:.78rem;font-weight:700;text-decoration:none;transition:all .2s;
                  {{ $currentStatus === 'rejected' ? 'background:var(--bg-card);color:#E05555;box-shadow:0 1px 4px rgba(0,0,0,.2);' : 'color:var(--text-muted);' }}">
            ❌ Refusées
        </a>
    </div>
</div>
...
@if($pending->isEmpty())
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:18px;padding:60px 32px;text-align:center;">
        ...
        <div style="font-family:var(--font-head);font-size:1rem;font-weight:700;color:var(--text);margin-bottom:8px;">
            ...
            @elseif($currentStatus === 'approved') Aucune candidature approuvée
            @else Aucune candidature refusée
            @endif
        </div>
        <div style="font-size:.83rem;color:var(--text-muted);">
            @if($currentStatus === 'pending') Tout est traité, bien joué !
            @else Aucun résultat pour ce filtre.
            @endif
        </div>
    </div>
@else
    <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:16px;padding:24px;transition:border-color .2s;" onmouseover="this.style.borderColor='rgba(255,255,255,.14)'" onmouseout="this.style.borderColor='var(--border)'">
            <div style="display:flex;align-items:flex-start;gap:20px;flex-wrap:wrap;">
                    ...
                    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:4px;">
                        ...
                        @if($user->country_type === 'senegal')
                            <span style="font-size:.75rem;">
                                <svg width="16" height="11" viewBox="0 0 22 15" xmlns="http://www.w3.org/2000/svg" style="border-radius:2px;vertical-align:middle;">
                        ...
                        @endif
                        {{-- Badge statut pour les onglets approved/rejected --}}
                        @if($currentStatus === 'approved')
                            <span style="background:rgba(45,90,61,.2);color:#7DDF9A;font-size:.68rem;font-weight:700;padding:2px 8px;border-radius:100px;">✅ Approuvé</span>
                        @elseif($currentStatus === 'rejected')
                            <span style="background:rgba(224,85,85,.12);color:#E05555;font-size:.68rem;font-weight:700;padding:2px 8px;border-radius:100px;">❌ Refusé</span>
                        @endif
                    </div>
                ...
                <div style="display:flex;flex-direction:column;gap:8px;flex-shrink:0;">
                    <form method="POST" action="{{ route('admin.applications.approve', $user) }}">
                        @csrf @method('PATCH')
                        <button type="submit" style="width:140px;padding:9px 16px;background:#2D5A3D;color:#7DDF9A;border:1px solid rgba(125,223,154,.25);border-radius:10px;font-family:var(--font-head);font-size:.82rem;font-weight:700;cursor:pointer;transition:all .2s;" onmouseover="this.style.background='#3A7A50'" onmouseout="this.style.background='#2D5A3D'">
                            ✅ Approuver
                        </button>
                    </form>
                    <button onclick="toggleRejectForm({{ $user->id }})" style="width:140px;padding:9px 16px;background:rgba(224,85,85,.1);color:#E05555;border:1px solid rgba(224,85,85,.25);border-radius:10px;font-family:var(--font-head);font-size:.82rem;font-weight:700;cursor:pointer;transition:all .2s;">
                        ❌ Refuser
                    </button>
                </div>
                @endif
